...
SRC_PKGS := apis client crds # directories which hold app source (not vendored)
...
label-crds: $(BUILD_DIRS)
	@for f in crds/*.yaml; do \
		echo "applying app.kubernetes.io/name=virttual-secrets label to $$f"; \
		kubectl label --overwrite -f $$f --local=true -o yaml app.kubernetes.io/name=virttual-secrets > bin/crd.yaml; \
		mv bin/crd.yaml $$f; \
	done
